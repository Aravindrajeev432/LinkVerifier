...
def main():
    ...
    discord_link_file_path = "all_discord_links.json"
    discord_links_from_from_file = read_contacts_from_json(discord_link_file_path)
    discord_link_slug_dict = dict()
    for link in discord_links_from_from_file:
        discord_link_slug_dict[link.get('slug')] = link.get('discord_url')

    
    coins_from_file = read_contacts_from_json(json_file_path)
    if coins_from_file.get('data').get('topCollectionsByCategory') is None:
        coins = coins_from_file.get('data').get('trendingCollectionsByCategory').get('edges')
    elif coins_from_file.get('data').get('trendingCollectionsByCategory') is None:
    ...
    for coin in tqdm(coins):
        ...
        # print(f"==>> slug: {slug}")
        if slug in discord_link_slug_dict:
            # discord_urls.append({"slug":slug,"discord_url":discord_link_slug_dict.get(slug)})
        ...
        if link:
            ...
            discord_link_slug_dict[slug] = link
        else:
            print(f"No discord link found  {slug}")
            # discord_urls.append({"slug":slug,"discord_url":None})
            discord_link_slug_dict[slug] = None

    for slug,link in discord_link_slug_dict.items():
        discord_urls.append({"slug":slug,"discord_url":link})
